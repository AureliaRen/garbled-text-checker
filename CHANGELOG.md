# Changelog

## [1.4.0] - 2026-09-10

### Added
- 第 13 种类型「公式码」：终端残留的未渲染 LaTeX 标记（`$$…$$`、`\frac{}{}`、`\times`、`10^{23}`、`\quad` 等）拉平为可读纯文本/Unicode——终端不渲染数学公式，按 LaTeX 写公式输出时会原样漏出
- 新脚本 `scripts/latex_to_text.py`（单文件零依赖）：定界符剥除、`\frac`/`\sqrt` 拉平、上下标转 Unicode 上下标（`10^{23}`→`10²³`）、希腊字母与关系符替换（`\alpha`→α、`\Leftrightarrow`→⇔）、`\text{}` 保留内容、残余命令兜底剥反斜杠
- `--check` 检测模式（有残留 exit 1，CI 友好）、`--demo` 自检（8 项断言）、文本/文件/管道三种输入
- SKILL.md 新增公式码节与输出侧预防规则（终端/聊天公式一律纯文本记法，如 `pV = (m/M)RT`、`10²³`、`⇔`）

### Notes
- 防误报设计：裸 `_x`/`^x` 仅转换数字和 +/-（`file_name`、`snake_case` 不受影响）；`\begin{}` 环境不解析，走兜底规则
- hook 有意不感知公式码：合法 LaTeX 文档（论文/讲义）大量含 `$$`，挂到每次工具输出上必然误报；公式码检测交给 CLI/skill 按需执行

## [1.3.0] - 2026-09-09

### Added
- 第 12 种类型「隐形码」：不可见 Unicode 检测与剥离——零宽符（U+200B/200C/2060/FEFF）、bidi 控制符（U+202A-E/2066-9）、tag 字符（U+E0000-E007F）、变体选择符，AI 水印与隐形提示注入的常见载体
- `fix()` 对隐形码的动作是剥离（strip-invisible）而非反向还原，输出分类统计标签（如 `零宽x1+bidi/运算符x1+tagx1`）
- 防误报设计：文件头 BOM、emoji ZWJ 序列、emoji 后 VS16 不报；游离变体选择符 <3 个视为排版噪音

## [1.2.0] - 2026-09-09

### Changed
- SKILL.md 不再内嵌便携脚本：提取为 `scripts/garbled_portable.py`，skill 触发时的加载体积约 -60%（脚本内容不进模型上下文，只读运行输出）
- description 增补触发信号（UnicodeEncodeError / 'gbk' codec / 锟斤拷 / 重音字母等），脱离用户侧全局配置也能正确触发
- install.sh 同步部署便携版；hook 提示文案与 README 用法同步更新
- 回归测试改为直接加载 `scripts/garbled_portable.py`（不再从 SKILL.md 提取）

### Fixed
- CI 冒烟步骤在 Windows runner 上失败：`/tmp` 为 POSIX 路径，改用 `shell: python` + `tempfile.gettempdir()` 跨平台兼容（自 v1.1.0 起即存在）

## [1.1.0] - 2026-08-15

### Added
- 完整版 CLI `scripts/garbled_fix.py`：多文件、目录递归扫描、`--json` 输出、`--check` 退出码模式、`--demo` 演示样本
- 5 种扩展乱码类型：HTML 双重转义、孤立代理项、cp1252 误读、UTF-16 误读、C1 控制符
- GitHub Actions CI（Ubuntu / Windows / macOS × Python 3.10/3.12）
- MIT License、双语 README

### Changed
- 符号码/拼音码检测重构为 latin-1 扩展字符占比判定
- 古文码检测改为假名/CJK 兼容符佐证（不依赖 GBK 可编码性）
- `clean_score` 双路径评分 + 全文本 GBK 校验，杜绝伪还原
- 口字码/锟拷码/C1 控制码不可还原时如实报告

### Fixed
- BFS 伪还原问题（谚文/私有区混入、纯拉丁伪候选、U+FFFD 痕迹）
- Windows 控制台 GBK 输出乱码（stdout 强制 UTF-8）

## [1.0.0] - 2026-08-14

### Added
- 六种经典乱码类型检测与修复（古文码/口字码/符号码/拼音码/问句码/锟拷码）
- Claude Code skill（SKILL.md 便携版内嵌脚本）
- PostToolUse 自动检测 hook（分层信号防误报）
